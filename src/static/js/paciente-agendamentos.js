...
function renderDiaryEntry(entry){const diaryListEl=document.getElementById('diaryList');if(!diaryListEl)return;const createdISO=entry.data_registro||entry.created_at||entry.createdAt||entry.dataHora;const d=createdISO?new Date(createdISO):null;const formattedDate=d?d.toLocaleDateString('pt-BR',{weekday:'long',year:'numeric',month:'long',day:'numeric'}):'Data não informada';const formattedTime=d?d.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'';const intensidade=typeof entry.intensidade==='number'?entry.intensidade:Number(entry.intensidade||0);const badgeClass=intensidade>=7?'text-bg-danger':(intensidade>=4?'text-bg-warning':'text-bg-success');const card=document.createElement('div');card.className='card shadow-sm mb-3';card.innerHTML=`<div class="card-body py-3"><div class="d-flex justify-content-between align-items-center"><div><div class="fw-normal">${formattedDate}</div><div class="text-muted"><i class="bi bi-clock"></i> ${formattedTime}</div></div><span class="badge rounded-pill ${badgeClass}">Intensidade: ${intensidade}</span></div><div class="row gy-2 mt-2"><div class="col-12"><div class="small text-muted">Situação</div><div class="fw-normal">${escapeHtml(entry.situacao)||'—'}</div></div><div class="col-12"><div class="small text-muted">Pensamento</div><div class="fw-normal">${escapeHtml(entry.pensamento)||'—'}</div></div><div class="col-md-6"><div class="small text-muted">Emoção</div><div class="fw-normal">${escapeHtml(entry.emocao)||'—'}</div></div><div class="col-md-6"><div class="small text-muted">Comportamento</div><div class="fw-normal">${escapeHtml(entry.comportamento)||'—'}</div></div>${entry.consequencia?`<div class="col-12"><div class="small text-muted">Consequência</div><div class="fw-normal">${escapeHtml(entry.consequencia)}</div></div>`:''}</div></div>`;diaryListEl.appendChild(card);}function escapeHtml(text){if(text==null)return'';return String(text).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;');}
});
function updateDashboardNextAppointment(appointments){
  ...
  appointments.forEach(appt=>{
    ...
    if(chosen){
      if(!candidate || chosen < candidate.date){
        candidate={date:chosen, profissional: appt.funcionario_nome||'N/A'};
      }
    }
  });
  if(!candidate){
  ...
  nextAppointmentContent.innerHTML=`<i class="bi bi-calendar-event"></i> ${dateStr} às ${timeStr} <span class="ms-2 badge rounded-pill text-bg-secondary">Profissional: ${escapeHtml(candidate.profissional)}</span>`;
}
function updateDashboardLatestDiary(entry){
  ...
  latestDiaryContent.innerHTML=`<i class="bi bi-clock"></i> ${dateStr} às ${timeStr} <span class="ms-2 badge rounded-pill ${intensidade>=7?'text-bg-danger':(intensidade>=4?'text-bg-warning':'text-bg-success')}">Intensidade: ${intensidade}</span><div class="mt-1"><span class="small text-muted">Emoção: </span><span class="fw-normal">${emocao}</span></div>`;
}